...
def make_random_policy(np_random):
    def random_policy(state, player_color):
        possible_places = OthelloEnv.get_possible_actions(state, player_color)
        # No places left
...
class OthelloEnv(gym.Env):
    ...
    def _reset(self):
        ...
        self.possible_actions = OthelloEnv.get_possible_actions(self.state, self.to_play)
        self.done = False
        ...
        if self.player_color != self.to_play:
            ...
            OthelloEnv.make_place(self.state, a, OthelloEnv.BLACK)
    ...
    def _step(self, action):
        ...
        if OthelloEnv.pass_place(self.board_size, action):
            pass
        elif OthelloEnv.resign_place(self.board_size, action):
            return self.state, -1, True, {'state': self.state}
        elif not OthelloEnv.valid_place(self.state, action, self.player_color):
            ...
            OthelloEnv.make_place(self.state, action, self.player_color)
        ...
        if a is not None:
            if OthelloEnv.pass_place(self.board_size, a):
                pass
            elif OthelloEnv.resign_place(self.board_size, a):
                return self.state, 1, True, {'state': self.state}
            elif not OthelloEnv.valid_place(self.state, a, 1 - self.player_color):
                if self.illegal_place_mode == 'raise':
                    raise
                elif self.illegal_place_mode == 'lose':
                    # Automatic loss on illegal place
                ...
                OthelloEnv.make_place(self.state, a, 1 - self.player_color)


        self.possible_actions = OthelloEnv.get_possible_actions(self.state, self.player_color)
    ...
    def valid_place(board, action, player_color):
        ...
        if board[2, coords[0], coords[1]] == 1:
            # check whether there is any reversible places
            if OthelloEnv.valid_reverse_opponent(board, coords, player_color):
                return True
            else:
                return False
        else:
